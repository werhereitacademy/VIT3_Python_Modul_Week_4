import json
import os
from book_file import delete_book, record, read
...
def borrow_book():
    while True:
        ...
        book_found = False
        for book in books:
            if book["Kitap_Adi"] == book_borrow_choice:
                book_found = True
                break
            ...
            try:
                ...
                for member in members:
                    if member["id"] == id:
                        ...
                        new_books = [k for k in books if k["Kitap_Adi"] != book_borrow_choice]
                        record(new_books)
...
def return_book():
    ...
    for loan in track_file:
        if loan["Member"]["id"] == id:
            ...
            book_info = loan["Book"]
            print(
                f"Book in your possession: Book Title: {book_info['Kitap_Adi']} - Author: {book_info['Yazar']} - Publisher: {book_info['Yayinevi']}")
            print("-" * 30)
    ...
    while True:
        ...
        if return_book_name == "0":
            return
        for loan in track_file:
            if loan["Member"]["id"] == id and loan["Book"]["Kitap_Adi"].lower() == return_book_name.lower():
                books = read()
...
def book_tracking():
    ...
    for loan in track_file:
        if loan["Member"]["id"] == id:
            ...
            book_info = loan["Book"]
            print(
                f"Books in your possession: Book Title: {book_info['Kitap_Adi']} - Author: {book_info['Yazar']} - Publisher: {book_info['Yayinevi']}")
            print("-" * 30)
        else:
